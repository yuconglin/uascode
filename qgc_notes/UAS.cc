...
void UAS::receiveMessage(LinkInterface* link, mavlink_message_t message)
{
    ...
    if (message.sysid == uasId && (!attitudeStamped || (attitudeStamped && (lastAttitude != 0)) || message.msgid == MAVLINK_MSG_ID_ATTITUDE))
    {
        ...
        case MAVLINK_MSG_ID_GLOBAL_VISION_POSITION_ESTIMATE:
        {
            ...
            break;
	case MAVLINK_MSG_ID_MULTI_ADSB3:
	{
            mavlink_multi_adsb3_t adsb3_t;
	    //std::cout << "adsb3 received" << "\n";
	    mavlink_msg_multi_adsb3_decode(&message, &adsb3_t);
            quint64 time= getUnixTime();
            /*
            for(int i = 0; i != adsb3_t.number; ++i)
	    {
               std::cout <<  (int)adsb3_t.addrs[i] << ' '
		       << (double)adsb3_t.lats[i] << ' '
		       << (double)adsb3_t.lons[i] << ' '
		       << (double)adsb3_t.yaws[i] << "\n";
	    }*/

	    emit adsb3Changed(this, 
	     adsb3_t.addrs[0], adsb3_t.addrs[1], adsb3_t.addrs[2], 
	     adsb3_t.lats[0], adsb3_t.lats[1], adsb3_t.lats[2], 
	     adsb3_t.lons[0], adsb3_t.lons[1], adsb3_t.lons[2],
	     adsb3_t.yaws[0], adsb3_t.yaws[1], adsb3_t.yaws[2], 
             adsb3_t.number, time);

	    emit adsbChanged(this, adsb3_t.addrs[0], adsb3_t.lats[0], adsb3_t.lons[0], adsb3_t.yaws[0]);
        }
	    break;
	    ...
	    //std::cout << "mavlink_global_position_int_t" << "\n";
