...
void QGCMapWidget::addUAS(UASInterface* uas)
{
    ...
    connect(uas, SIGNAL(
     adsb3Changed(UASInterface*, 
	     quint32, quint32, quint32, 
 	     float, float, float, 
	     float, float, float, 
	     float, float, float, 
	     quint8, quint64)), 
    ...
    connect(uas, SIGNAL(adsbChanged(UASInterface*, quint32, float, float, float)),this, SLOT(updateAdsb(UASInterface*, quint32, float,float,float)));

    std::cout << "after connecting adsb3" << "\n";
...
void QGCMapWidget::updateAdsb(UASInterface* uas, quint32 addr, float lat, float lon, float yaw)
{
    //std::cout << "update adsb" << "\n";
}

void QGCMapWidget::updateAdsb3(UASInterface* uas, 
	quint32 addr0, quint32 addr1, quint32 addr2, 
	float lat0, float lat1, float lat2, 
	float lon0, float lon1, float lon2, 
	float yaw0, float yaw1, float yaw2, 
	quint8 num, quint64 usec)
{
    std::cout << "updateAdsb3" << "\n";
    Q_UNUSED(usec);
    mapcontrol::UAVItem* uav;
    ...
    for(int i = 0; i != num; ++i)
    {
	    int color_idx= (ADSBs.size()>6)?6:ADSBs.size(); 
	    int red=ADSB_COLOR[color_idx][0];
	    int green=ADSB_COLOR[color_idx][1];
	    int blue=ADSB_COLOR[color_idx][2];

	    if (!ADSBs.contains(addrs[i])){
	       uav=new MAV2DIcon(map,this,lats[i],lons[i],720,QColor(red,green,blue));
	       uav->setParentItem(map);
	       ADSBs.insert(addrs[i], uav);
	       ADDRESS_COLORs.insert(addrs[i],QColor(red,green,blue));
	       // Set the correct trail type
	       uav->SetTrailType(trailType);
	       // Set the correct trail interval
	       if(trailType == mapcontrol::UAVTrailType::ByDistance)
		  ...
		  uav= ADSBs.value(addrs[i]); 
		  internals::PointLatLng pos_lat_lon = internals::PointLatLng(lats[i], lons[i]);
		  //color for the trail
		  uav->SetUAVPos(pos_lat_lon,720,ADDRESS_COLORs.value(addrs[i]));
	   }//if-else ends

	   // Set new lat/lon position of UAV icon
	   std::cout<<"address: "<< addrs[i] 
		   <<" lat: "<< lats[i] 
		   <<" lon: "<< lons[i]
	           <<" yaw: "<< yaws[i] << "\n";
	//	   <<"alt:"<< alt<<" yaw: "<<yaw<< std::endl;
	      // Convert from radians to degrees and apply
	   uav->SetUAVHeading(yaws[i]);
	   //draw icon
	   MAV2DIcon* icon = dynamic_cast<MAV2DIcon*>(uav);
	   
	   if (icon)
	   {
	       // Set new airframe
	       //std::cout<<"icon yes"<< std::endl;
	       icon->setAirframe(uas->getAirframe());
	       icon->drawIcon();
	   }
    }
