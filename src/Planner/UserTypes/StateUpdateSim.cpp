...
namespace UasCode{
                        ...
                        double dt)  
  {
      ...
      double Dpitch= K_pitch*Utils::_wrap_pi(dem_pitch-st_pre.pitch)
                   + ( L*cos(nav_roll)-Muav*CONSTANT_G*cos(st_pre.pitch) )/ (Muav*st_pre.speed);
      ...
      //the updated state
      UserStructs::PlaneStateSim st_new(st_pre.t+dt,x1,y1,0,0,z1,speed1,yaw1,dem_pitch,ax1,ay1,az1);
      st_new.GetGCS();
      return st_new;
  }
