...
namespace UasCode{
  ...
  //free function ends
   ...
   {  
      ...
      std::vector<UserStructs::AdsbQGC> qgc_adsbs;
	...
	if(msg_re== 10)
	{
	  ...
	  //if(mavlink_send) sender.SendAdsbMsg(msg);
	...
	if(msg_re== 20)
	{
	  ...
	  //send mavlink adsg message
          //if(mavlink_send) sender.SendAdsbMsg(msg);
	  //to short qgc adsb msg
	  UserStructs::AdsbQGC qgc_msg= Utils::AdsbMsgToQGC(msg); 
	  //if no repeats find, just insert it
	  bool repeat= false;
	  for(int i=0;i!=qgc_adsbs.size();++i)
	  {
             if(qgc_msg.address== qgc_adsbs[i].address){
               repeat= true;
	       break;
	     }
	  }
	  if(repeat) {
	    ...
	    if(mavlink_send) sender.SendMultiAdsbs(qgc_adsbs);
	    qgc_adsbs.clear();
	    obss.clear();
	    //ros publish
	  ...
	  qgc_adsbs.push_back(qgc_msg);
	 
	  //otherwise, clean the qgc_adsbs and then insert
          //short qgc parts end
